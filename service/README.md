...
Prompt 正文在 `catalog/`：`slots/system/` `slots/user/` `skills/` `window.system.md` `window.user.md` `assemble.json`。两份插槽清单为 system-slots.md / user-slots.md；模板只排顺序，各槽文件维护解释与数据占位。工具说明仅来自工具 JSON 的 function.description，index 只分类。system 是固定规则和常驻工具说明，user 是请求与参考数据。
