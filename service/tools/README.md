...
支持模块为 userInputHistory、pageObservedHistory、conversationMemory、toolIO。目录与原文由 compression 能力管理，工具层只校验参数并调度。查询 Agent 仅选择候选，返回 ID 不能作为文件路径。查询结果不再经过普通工具的二次截断。
