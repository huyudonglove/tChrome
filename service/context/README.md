...
本目录是本机服务的上下文能力模块，配置正文与实现放在一起。本文件面向人，不加载到模型窗口。模型导航与唯一模块顺序来源是 [System 栏目清单](system-slots.md) 和 [User 栏目清单](user-slots.md)。加载器按清单编号行顺序读取同名模块；调整顺序只改清单，编号须从 1 连续递增。清单与目录必须一一对应，缺失、重复或未列出的模块会报错。
...
| [tools/definitions/groups.json](../tools/definitions/groups.json) | `baseToolsIds` 常驻基础工具、`coreToolIds` 初始动态工具 | 工具加载分组 |
| [tools/definitions/index.json](../tools/definitions/index.json) | browser / service 分类，供发现可加载工具 | 工具目录 |
| [tools/definitions/](../tools/definitions/) | 每个 JSON 保存完整 schema；function.description 是唯一工具说明来源 | 工具 API |
| [modules.ts](modules.ts) | 读取清单、模块和 skill；按清单渲染 | 上下文加载层 |
| [registry.ts](../tools/registry.ts) | 读取 service/tools/definitions/ 的定义、分类和分组，生成工具说明 | 工具层 |
| [window.ts](window.ts) | 纯投影数据并拼装 system / user；两份清单原文仅放 system 一次 | 窗口层 |
| [messages.json](../runtime/messages.json) | 空回复、未调用结束工具时的运行提示；按需进入 toolIO | 运行层 |
...
本模块 `service/context/` 同时维护窗口正文、栏目顺序、skill、加载器和投影实现。工具 API 定义属于同一服务的 `service/tools/definitions/`。工具注册表提供 schema 和说明，Context 不读取工具目录、不执行工具，也不维护执行状态。
