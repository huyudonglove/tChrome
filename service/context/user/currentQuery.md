...
records 未超内联门禁时整段注入。超量时 records 为空数组，并带 externalized=true、preview、path、search=evidence.search；用 evidence.search(windows=[{callId: sourceCallId, keyword|startLine}]) 读取。
