...
发送给主模型前，Runtime 检查 System 和 User 的总字符数：达到 200000 字符时先压缩；压缩后仍超过 250000 字符时，优先把 #notes 正文保存到本地，用路径替换正文，再处理其他可裁剪的大块内容。#skill 始终保留全文，不参与压缩或裁剪；#baseTools、#tools 和编号规则保持内联。
