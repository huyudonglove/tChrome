...
const directory = async (dataDir: string) => {
...
export async function readScript(dataDir: string, filename: string): Promise<{filename: string; path: string; code: string}> {
  if (!validName(filename)) throw new Error('脚本文件名无效');
  const path = join(await directory(dataDir), filename);
  if (!await regular(path)) throw new Error('脚本不存在');
...
export async function patchScript(dataDir: string, input: {filename?: unknown; patch?: unknown}): Promise<Record<string, unknown>> {
  ...
  const operation = previous.catch(() => {}).then(async () => {
    ...
    try {
      ...
      if (!validName(filename)) throw new Error('脚本文件名无效');
      if (typeof patch !== 'string' || !patch.trim() || patch.length > 1_000_000) throw new Error('需要有效 unified diff，最多 1000000 字符');
      ...
      if (!result) {
        ...
        await rm(path);
        return {ok: true, filename, path, deleted: true};
      }
      ...
      return {ok: true, filename, path, code};
