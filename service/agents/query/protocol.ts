...
export async function requestMatches(input: {
  ...
  request: { module: string; tag: string; question: string };
  ...
  const response = await input.provider.complete({ tools: [tool], messages: [
    { role: "system", content: system }, { role: "user", content: JSON.stringify({ request: input.request, catalog: input.candidates }) },
  ] });
  ...
  const allowed = new Set(input.candidates.map(entry => entry.id));
  if ((value.ids as string[]).some(id => !allowed.has(id))) throw new Error("查询 Agent 返回了无效或目录外的 ID。");
