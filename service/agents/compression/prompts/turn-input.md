...
status 的 completed、waiting_human、failed 分别表示结束、等待用户、失败；assembling/inferring 表示运行中。createdAt/completedAt 是起止时间，completedAt=null 表示未提供收尾时间。sequence.turn/batch 用于排序。
...
- goalChanges：id 标识目标版本，goal 是正文，sourceCallId 关联调用，createdAt 是时间。空数组不表示没有持续目标。
