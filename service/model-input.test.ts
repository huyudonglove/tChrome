  ...
  const provider: Provider = { complete: async ({ messages, tools }) => {
    ...
    return { finish: "tool_calls", content: "", attempts: 1, parseOk: true, schemaOk: true,
      faultCode: null, missing: [], toolCalls: requests === 1
        ...
        : [{ id: "done", name: "finishTurn", arguments: { text: "完成", reason: "答复", affectsPage: false, summary: "完成"} }] };
  } };
  try {
    ...
    expect(next.output).toEqual({ kind: "reply", text: "完成", summary: "完成" });
    ...
    expect((await handleTurn({ dataDir, repoRoot: join(import.meta.dir, ".."), provider }, { userInput: "新会话", submittedAt: "now" })).output).toEqual({ kind: "reply", text: "完成", summary: "完成" });
    expect(requests).toBe(4);
    openConversation(dataDir, result.conversationId);
    expect((await handleTurn({ dataDir, repoRoot: join(import.meta.dir, ".."), provider }, { userInput: "重新打开", submittedAt: "now" })).output).toEqual({ kind: "reply", text: "完成", summary: "完成" });
