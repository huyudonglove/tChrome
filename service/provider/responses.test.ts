...
test("Responses sends flat tools and high reasoning and normalizes message plus function calls", async () => {
  ...
  const server = Bun.serve({ hostname: "127.0.0.1", port: 0, async fetch(request) {
    ...
    return Response.json({ status: "completed", output: [
      ...
      { type: "function_call", id: "item_1", call_id: "call_1", name: "finishTurn", arguments: '{"text":"完成","summary":"完成"}' },
    ] });
  } });
  try {
    const result = await createProvider({ api: "responses", apiKey: "test", baseURL: `http://127.0.0.1:${server.port}/v1`, model: "grok-4.6", proxy: "" }).complete({
      ...
      tools: [{ type: "function", function: { name: "finishTurn", parameters: { type: "object", properties: { text: { type: "string" }, summary: { type: "string" } } } } }],
    });
    ...
    expect(result).toMatchObject({ finish: "tool_calls", content: "执行下一步", toolCalls: [{ id: "call_1", name: "finishTurn", arguments: { text: "完成", summary: "完成"} }] });
