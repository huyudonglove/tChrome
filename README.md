...
侧栏顶栏的书签图标打开本地资料库，可收藏网站、账号和普通资料，搜索、编辑、复制及删除。资料跨会话保存，删除会话不会删除收藏，账号密码直接明文显示。Agent 可加载动态工具 `library` 使用 `list/get/save/delete` 操作同一份资料，保存后面板自动刷新；资料不自动整库注入模型上下文。
...
其中 `conversations/<会话ID>/` 保存会话账本、事件日志、`provider.md` 模型交互记录、Turn、会话记忆、会话历史压缩目录及工具完整返回；`memory/project/` 独立保存共享长期记忆。
