...
| `baseToolsIds` | 常驻工具，每轮都出网 | `askUser` `finishTurn` `tool.detail` `memory.write` |
...
| `baseToolsIds` | string[] | 装配器 | 常驻工具，对应 `catalog/tools/<id>.json`。本轮固定 `askUser` `finishTurn` `tool.detail` `memory.write` `memory.write` |
