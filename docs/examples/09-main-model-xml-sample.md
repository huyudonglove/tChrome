...
工具能力按大类组织。常驻能力见 <baseTools>（收口与提问、目标、记忆/笔记、本轮反思、检查清单、证据检索、catalog 与 skill 加载、浏览器主链路与 job 查询/停止）。技能正文按需 skill.load，清单见 <systemSkill>。动态能力默认需 catalog.add 加载，可用 list_browser_tools 查看名称，大致包括：
...
- skill.list：列出会话可用技能：id、tags、首句。
- skill.load：按 id 加载技能正文到本轮 <skill>，会话内保持。
...
<systemSkill>
能力：【System Skill Index】

详细描述：
技能正文按需加载，不全部写入 <skill>。清单每行：id｜tags｜首句。用 skill.list（可选 tag 过滤）查看，用 skill.load(id) 载入本轮 <skill>；加载状态在会话内保持。系统级技能说明写在本模块；会话技能正文只出现在 User <skill>。
...
详细描述：
本会话已加载的技能正文。尚未加载时本栏为空；先 skill.list 再 skill.load(id)。加载后正文整段保留，不参与压缩。
