...
发送前达到 200000 字符触发压缩，达到 250000 字符时将 notes 正文保存到本地并替换为文件路径；处理后仍达到硬门槛才返回 context_limit。压缩外层保留 history/current 阶段，各阶段选中的材料完整请求一次，不在 Agent 内分批或递归压缩。查询候选同样完整请求一次。验证覆盖运行时发送前触发、近期记录保留、批量逐轮结果、取消和归档回查；模拟模型测试不能证明真实模型的摘要与匹配质量。
