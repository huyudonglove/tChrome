...
注入的 `{{archiveFields}}` 只列字段，不再加「注册表字段对照（modules.json compress=true）」标题。
...
字段语义在 <compressionModules>。turns 按历史顺序排列，一次通常含多个 turn。空数组或 null 只表示本次未提供内容，不证明历史上从未存在。

status：completed / waiting_human / failed 表示已结束；assembling / inferring 表示仍在运行。只总结材料里已有的内容，不补写缺失模块或未知结局。
</compressionTurns>
```
